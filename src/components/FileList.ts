...
export const getFileElement = (uuid: string): HTMLElement | undefined => {
  return document.querySelector(`li[id="${uuid}"]`) as HTMLElement
}

export const resetFileElementContent = (file: SyncedFile, elem?: HTMLElement, uuid?: string) => {
  elem ??= getFileElement(uuid ?? file.uuid)
  if (!elem) return
